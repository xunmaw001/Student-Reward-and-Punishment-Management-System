    ...
    */
    ...
    public R save(@RequestBody ChengjiEntity chengji, HttpServletRequest request){
        ...
        if(false)
            ...
            XueshengEntity xueshengEntity = xueshengService.selectById(chengji.getXueshengId());
            if(xueshengEntity == null)
                return R.error("查不到学生");
            xueshengEntity.setXueshengFenshu(xueshengEntity.getXueshengFenshu()+chengji.getChengjiDedaoFenshu());
            ...
            fenshujiluEntity.setXueshengId(chengji.getXueshengId());
            fenshujiluEntity.setFenshujiluFenshu(chengji.getChengjiDedaoFenshu());
            fenshujiluEntity.setFenshujiluContent("得到"+chengji.getChengjiDedaoFenshu()+"分");
            fenshujiluEntity.setFenshujiluTypes(4);
...
//            chengji.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ChengjiEntity> queryWrapper = new EntityWrapper<ChengjiEntity>()
            ...
            .eq("chengji_dedao_fenshu", chengji.getChengjiDedaoFenshu())
            ;
...
//                            chengjiEntity.setChengjiFenshu(Integer.valueOf(data.get(0)));   //成绩分数 要改的
//                            chengjiEntity.setChengjiDedaoFenshu(Integer.valueOf(data.get(0)));   //所得分数 要改的
