        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '记录类型' : ''">
                        <el-select v-model="searchForm.fenshujiluTypes" placeholder="请选择记录类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in fenshujiluTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('fenshujilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xueshengjiangchengxitong/upload/fenshujiluMuBan.xls"
                        >批量导入分数记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('fenshujilu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="fenshujiluUploadSuccess"
                                :on-error="fenshujiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('fenshujilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入分数记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('fenshujilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "fenshujilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="fenshujiluTypes"
                                      header-align="center"
                                      label="记录类型">
                        <template slot-scope="scope">
                            {{scope.row.fenshujiluValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="fenshujiluFenshu"
                                      header-align="center"
                                      label="记录分数">
                        <template slot-scope="scope">
                            {{scope.row.fenshujiluFenshu}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('fenshujilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('fenshujilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('fenshujilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              fenshujiluTypesSelectSearch : [],
            form:{
                id : null,
                xueshengId : null,
                fenshujiluTypes : null,
                fenshujiluFenshu : null,
                fenshujiluContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '分数': 'xueshengFenshu',
                //本表字段
                     '记录类型': "fenshujiluTypes",
                     '记录分数': "fenshujiluFenshu",
                     '记录时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'fenshujilu',//当前表表名,
                        sumColum : 'fenshujilu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'fenshujilu_name',//分组字符串字段
                        // types : 'fenshujilu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.fenshujiluTypes!= '' && this.searchForm.fenshujiluTypes!= undefined) {
                    params['fenshujiluTypes'] = this.searchForm.fenshujiluTypes
                }
                                
                params['fenshujiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=fenshujilu_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.fenshujiluTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            fenshujiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "fenshujilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
