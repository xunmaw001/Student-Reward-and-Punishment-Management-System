        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '出勤率编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.chuqinlvUuidNumber" placeholder="出勤率编号" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '月份' : ''">
                         <el-date-picker
                                 v-model="searchForm.chuqinlvYue"
                                 type="month"
                                 value-format="yyyy-MM"
                                 placeholder="月份">
                         </el-date-picker>
                         <!--<el-input prefix-icon="el-icon-search" v-model="searchForm.chuqinlvYue" placeholder="月份" clearable></el-input>-->
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('chuqinlv','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xueshengjiangchengxitong/upload/chuqinlvMuBan.xls"
                        >批量导入出勤率数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('chuqinlv','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="chuqinlvUploadSuccess"
                                :on-error="chuqinlvUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('chuqinlv','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "chuqinlv.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="chuqinlvUuidNumber"
                                   header-align="center"
                                   label="出勤率编号">
                        <template slot-scope="scope">
                            {{scope.row.chuqinlvUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('chuqinlv','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('chuqinlv','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('chuqinlv','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                xueshengId : null,
                chuqinlvUuidNumber : null,
                chuqinlvYue : null,
                chuqinlvName : null,
                chuqinlvFenshu : null,
                chuqinlvContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '分数': 'xueshengFenshu',
                //本表字段
                     '出勤率编号': "chuqinlvUuidNumber",
                     '月份': "chuqinlvYue",
                     '出勤率': "chuqinlvName",
                     '出勤率分数': "chuqinlvFenshu",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'chuqinlv',//当前表表名,
                        sumColum : 'chuqinlv_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'chuqinlv_name',//分组字符串字段
                        // types : 'chuqinlv_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.chuqinlvUuidNumber!= '' && this.searchForm.chuqinlvUuidNumber!= undefined) {
                    params['chuqinlvUuidNumber'] = '%' + this.searchForm.chuqinlvUuidNumber + '%'
                }
         
                if (this.searchForm.chuqinlvYue!= '' && this.searchForm.chuqinlvYue!= undefined) {
                    params['chuqinlvYue'] = '%' + this.searchForm.chuqinlvYue + '%'
                }
                                        
                params['chuqinlvDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            chuqinlvUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "chuqinlv/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
